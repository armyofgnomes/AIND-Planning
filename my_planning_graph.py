...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check node_a1's add effects in node_a2's negative preconditions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True

        # Check node_a1's remove effects in node_a2's positive preconditions
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True

        # Check node_a2's add effects in node_a2's negative preconditions
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # Check node_a2's remove effects in node_a1's positive preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure that a1's preconditions aren't mutually exclusive from a2's
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        return node_s1.symbol == node_s1.symbol and node_s1.is_pos != node_s2.is_pos
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Ensure that s1's preconditions aren't mutually exclusive from s2's
        # Similar to competing_needs_mutex, but with opposite returns and checking
        # for negation
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for level, pg_nodes in enumerate(self.s_levels):
                if PgNode_s(goal, True) in pg_nodes:
                    level_sum += level
                    break
        return level_sum
